...
$(function(){
	...
	$("input[name='username']").blur(function(){
		...
		$.post(CONTROL+"/checkAccount",{username:username.val().trim()},function(data){
			if(data.status){
				validate.username=0;
	...
	$("input[name='password']").blur(function(){
		...
		$.post(CONTROL+"/checkpassword",{password:password.val().trim(),username:username.val().trim()},function(data){
			if(data.status){
				validate.password=0;
